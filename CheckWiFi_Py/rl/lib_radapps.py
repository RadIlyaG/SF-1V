...
class Sftp:
    ...
    def Open(self):
        ...
        try:
            ...
            print(f"\n{datetime.datetime.now()} before connect")
            self.ssh.connect(self.host, username=self.username, password=self.password)  # , timeout=200
            print(f"after: {datetime.datetime.now()}")
            self.sftp = self.ssh.open_sftp()            
        except Exception as excpt:
            print(f'{datetime.datetime.now()} Sftp.Open.Exp:{excpt}') 
            return False
        return True
